...
- review hardening uses exactly four reviewer roles (`correctness`, `security`, `maintainability`, `project-standards`) with parallel subagent dispatch when available and sequential fallback when unavailable; Codex runtimes must explicitly discover and use `multi_agent_v1.spawn_agent`/`wait_agent` rather than simulating reviewer personas inline
