---
name: forge-review-plan
description: Review a completed plan for alignment and hardening before implementation.
---
Read fully:
- `memory.md`
- `research.md`
- `plan.md`
- `todo.json`
Read `forge-session.json` when present and refresh it after the review.
1. summarize the full intent chain
2. run the alignment coverage pass across research, plan, and todo
3. auto-sync clerical fidelity repairs that preserve approved intent
4. present the alignment packet
5. run hardening critique
6. present actionable findings with concrete mitigation sets
7. persist normalized findings to `research.md` and `forge-session.json`
Show before hardening:
- artifact intake summary
- alignment status counts
- auto-synced fidelity repairs
- ranked actionable alignment findings
- unsupported assumptions or scope mismatches
- non-blocking hygiene debt
- if `todo.json` uses legacy `items`, carries completed tasks from a superseded scope, or its refs no longer resolve, regenerate `todo.json` before implementation handoff
- no critique before alignment packet
- no asking permission for clerical auto-sync
- no implementation changes
